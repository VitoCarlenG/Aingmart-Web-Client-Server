<?php
    ...
    if(empty($_POST["nama"]) && empty($_POST["email"]) && empty($_POST["password"])) {
        echo"
            ...
            <script>
            window.location = '../page/createPenggunaPage.php'
            </script>";
    ...
    if(mysqli_num_rows($query) > 0) {
        echo '<script>alert("Email sudah terdaftar"); window.location = "../page/createPenggunaPage.php"</script>';
    } else {
        ...
        if (!$mail->send()) {
            ...
            echo
                '<script>alert("Create User Success, Check The Email For Verification"); window.location = "../page/listPenggunaPage.php"</script>';
            //Section 2: IMAP
